...
public:
    ...
    point north() const override {
        return point((w.xo + e.xo) / 2, e.yo < w.yo ? w.yo : e.yo);
    }
    point south() const override {
        return point((w.xo + e.xo) / 2, e.yo < w.yo ? e.yo : w.yo);
    }
    point east() const override {
        return point(e.xo < w.xo ? w.xo : e.xo, (w.yo + e.yo) / 2);
    }
    point west() const override {
        return point(e.xo < w.xo ? e.xo : w.xo, (w.yo + e.yo) / 2);
    }
    point neast() const override {
        return point(w.xo < e.xo ? e.xo : w.xo, e.yo < w.yo ? w.yo : e.yo);
    }
    point seast() const override {
        return point(w.xo < e.xo ? e.xo : w.xo, e.yo < w.yo ? e.yo : w.yo);
    }
    point nwest() const override {
        return point(w.xo < e.xo ? w.xo : e.xo, e.yo < w.yo ? w.yo : e.yo);
    }
    point swest() const override {
        return point(w.xo < e.xo ? w.xo : e.xo, e.yo <w.yo ? e.yo : w.yo);
    }
    void move(int a, int b) override {
        w.xo += a; w.yo += b; e.xo += a; e.yo += b;
    }
    void draw() override {
        try {
            put_line(w, e);
...
public:
    ...
    point north() const override {
        return point((sw.xo + ne.xo) / 2, ne.yo);
    }
    point south() const override {
        return point((sw.xo + ne.xo) / 2, sw.yo);
    }
    point east() const override {
        return point(ne.xo, (sw.yo + ne.yo) / 2);
    }
    point west() const override {
        return point(sw.xo, (sw.yo + ne.yo) / 2);
    }
    point neast() const override {
        return ne;
    }
    point seast() const override {
        return point(ne.xo, sw.yo);
    }
    point nwest() const override {
        return point(sw.xo, ne.yo);
    }
    point swest() const override {
        return sw;
    }
    void rotate_right() override { // Поворот вправо относительно se
        int w = ne.xo - sw.xo,
                h = ne.yo - sw.yo; // (учитывается масштаб по осям)
        sw.xo = ne.xo - h * 2;
        ne.yo = sw.yo + w / 2;
    }
...
void pos3(shape& p, const shape& q) {
    point n = q.east();
    point s = p.seast();
    p.move(n.xo + s.xo - 7, s.yo + 6);
}

void pos7(shape& p, const shape& q) {
    point n = q.nwest();
    point s = p.swest();
    p.move(s.xo - n.xo + 1, n.yo + s.yo);
}
